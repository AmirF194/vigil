...
typed decision, a **worker** — one of a registry of specialists — that turns a
query intent into evidence, and a **critic** that argues the strongest benign
explanation before anything may be called proven. What each role is told, must
answer in, and may call is declared in YAML, so a different workflow is a config
...
| `vigil.config.yaml` | `model`, `rates`, `budgets`, `runtime`, `verdicts`, `tools`, `enrichment` | operator |
...
use for — but never widen it, and it may only declare verbs the controller acts
on. A verb that would be journaled and ignored is rejected at load rather than
becoming a dead end the lead keeps choosing: it would cost an iteration and move
nothing. `EXECUTABLE_ACTIONS` is that list, and it grows as the controller learns
a verb — `CHECKPOINT` and `HANDOFF_IR` arrive with the checkpoint machinery.
...
and recorded on its dispatch row with the tool calls it ran), and the critic,
including what a call that died mid-way had already spent.
...
tool queried twice is one system agreeing with itself, and two specialists over
the same table are one source. Each worker names the telemetry domain a finding
came from, narrowed at spec build to the playbook's `data_domains`, and a label
the hunt never declared collapses into one bucket rather than buying
independence credit.
