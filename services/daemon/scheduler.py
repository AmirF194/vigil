...
class TaskScheduler:
    ...
    def __init__(self, config: SchedulerConfig):
        ...
        # The orchestrator's intake, handed over by the daemon. Everything past
        # this point -- the run, its budget, its projection -- is already the
        # orchestrator's, so a scheduled hunt is one more item on its queue.
    ...
    async def _run_threat_hunt(self):
        ...
        record, the budget and the reconcile against the run's own projection,
        and a second path to any of those would be a second set of guardrails.
        """
        logger.info("Starting scheduled threat hunt...")
        self.stats["threat_hunts"] += 1

        if self._investigation_queue is None:
            logger.warning("No investigation queue; the scheduled hunt cannot be opened")
            return
        ...
        logger.info("Queued a scheduled threat hunt: %s", hypothesis or "the definition's hypotheses")

    # What this hunt is out to test, beyond what the definition already states.
    # Read off the techniques the estate is actually showing, so a nightly hunt
    # follows the estate rather than repeating one fixed question forever.
