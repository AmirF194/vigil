...
class Orchestrator:
    ...
    async def _create_manual_investigation(
        ...
        await self._create_investigation(
            workflow_id=workflow_id,
            findings=findings,
            # The intake is shared, so what put the item on it is the item's to
            # say: a nightly hunt and an analyst's request are not the same event.
    ...
    async def _create_investigation(
        ...
        # Beside the context and read back the same way at enqueue. A hunt tests
        # beliefs, so what this one was opened to test has to reach its board as
        # a hypothesis rather than as prose inside the brief.
        if hypothesis:
    ...
    async def _enqueue_investigation(self, inv_record: Dict) -> None:
        ...
        request = {
            ...
            # One per line, as the console's run modal also sends them. Empty for
            # a workflow that walks phases, which has no board to put them on.
            "hypotheses": [
                line.strip()
                for line in (self.workdir.read_file(inv_id, "hypotheses.txt") or "").splitlines()
                if line.strip()
            ],
