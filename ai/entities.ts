...
// key id will not match anything an analyst searches for.
...
// paths share this: a key claiming a type does not make its value one either, and
// a "-" placeholder or a null rollup is everywhere in telemetry.
...
// being read as a domain: no pattern can tell kernel32.dll from froth.ly, but
// the key it arrived under can.
...
// it questions rather than reaching past it into raw evidence.
...
class Graph implements EntityGraph {
  ...
  // promoting those would fire on nearly every record, which ranks nothing.
  introducedRecurring(record: EvidenceRecord): boolean {
