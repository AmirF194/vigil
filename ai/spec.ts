...
// specialist is a YAML block rather than a change here. The critic is optional —
// an arch without one runs a whole hunt, it simply cannot reach proven and says
// so at each VALIDATE rather than transitioning quietly.
...
const ROLE_GROUPS = new Set(["lead", ALL_WORKERS, "workers_preamble", "critic"]);
...
function applyDirectives(roles: Roles, directives: Record<string, string>, declared: Set<string>): Roles {
  ...
  if (roles.critic !== undefined) {
    applied.critic = extend(roles.critic, "critic", [directives["critic"]], declared);
  }
  return applied;
}

// Corroboration is counted over source systems, so a worker may not invent one:
// a hunt that declares its telemetry domains gets them as the closed enum every
// worker must answer in. Two labels for the same DuckDB file would otherwise buy
// independence credit the data never earned.
function narrowSources(roles: Roles, dataDomains: readonly string[]): Roles {
  if (dataDomains.length === 0) return roles;

  const workers = Object.entries(roles.workers).map(([id, role]) => {
    const schema = structuredClone(role.output_schema);
    const where = `roles.workers.${id}.output_schema`;
    const results = asRecord(asRecord(schema["properties"], `${where}.properties`)["results"], `${where}.results`);
    const properties = asRecord(asRecord(results["items"], `${where}.results.items`)["properties"], `${where}.items`);

    if (properties["source_system"] === undefined) {
      throw new SpecError(
        `this playbook declares data_domains, so ${where} must declare results[].source_system ` +
          "for the worker to name the domain each finding came from",
      );
    }
    properties["source_system"] = { ...asRecord(properties["source_system"], "source_system"), enum: [...dataDomains] };
    return [id, { ...role, output_schema: schema }] as const;
  });
