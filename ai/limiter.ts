...
import type { RateLimit } from "./spec.js";
...
// Shared across every worker: RPM and TPM buckets, a concurrency gate, and
// jittered backoff so parallel workers that hit a 429 together do not stay
// synchronized on the retry.
  ...
  private async withRetry<T>(call: () => Promise<T>): Promise<T> {
    ...
    for (let attempt = 0; attempt < this.attempts; attempt += 1) {
      try {
        ...
        // 402 is Bifrost saying the budget is gone; retrying cannot help.
        if (status === 402) throw new BudgetExceeded((error as Error).message);
