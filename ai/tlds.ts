...
// No pattern encodes which suffixes are real, and without this list every
// kernel32.dll in a Sysmon dataset becomes a domain node.
const IANA = [
...
// backup.zip is a file far more often than a domain, so prose drops these; a
// value under a domain-shaped payload key is still typed correctly.
const FILE_EXTENSIONS = new Set(["zip", "mov", "js", "py", "sh", "pl", "cab", "prof"]);

export const TLDS: ReadonlySet<string> = new Set(IANA.filter((tld) => !FILE_EXTENSIONS.has(tld)));
