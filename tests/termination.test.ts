...
import { scoredFrontier } from "../ai/digest.js";
...
import { buildReport, renderReport, reportPath, type HuntReport } from "../ai/report.js";
import {
...
import { terminationVerdict } from "../ai/termination.js";
import type { Budgets, Decision, LinkRelation } from "../ai/types.js";
...
function newLedger(
  ...
  overrides: { budgets?: Budgets; termination?: Partial<Termination> } = {},
...
function controllerFor(ledger: Ledger, decisions: ScriptedDecision[], critic?: ScriptedDisconfirmationCritic) {
...
function resolve(ledger: Ledger, hypothesisId: string, status = "parked"): void {
...
function question(
  ledger: Ledger,
  question: string,
  fields: { entity_key?: string | null; spawned_iteration?: number; status?: "open" | "closed" } = {},
): string {
...
let unanswered = 0;
function gapOn(ledger: Ledger, hypothesisId: string): void {
  ...
  ledger.append({
    ...
    dispatch: {
      ...
      status: "failed",
      query_intent: `question ${(unanswered += 1)}`,
...
// cleared. This is what data_starved is made of.
async function gapLock(ledger: Ledger, hypothesisId: string): Promise<void> {
  const citations = [
    evidenceOn(ledger, hypothesisId, "cloudtrail"),
    evidenceOn(ledger, hypothesisId, "duckdb"),
  ];
  ...
  await controllerFor(
    ...
    [
      {
        action: "VALIDATE",
        rationale: "put it up for a verdict",
        target_hypothesis_id: hypothesisId,
...
describe("the predicate, not the recommendation", () => {
  ...
  it("completes and auto-parks the sub-floor leads as backlog", async () => {
    ...
    expect(report.backlog.map((entry) => entry.question)).toEqual(["recheck 10.0.0.9 next quarter"]);
  });
  ...
  it("reads gap-lock off the strength snapshot, not the resolution_reason prose", () => {
    const { ledger, hypothesisIds } = newLedger();
    ...
    const verdict = terminationVerdict(ledger.projection, 1, DEFAULT_TERMINATION, DEFAULT_VERDICTS);
  ...
  it("keeps completed when something was proven, however blind the rest of the hunt was", async () => {
    const { ledger, hypothesisIds } = newLedger(["h one", "h two"]);
    await gapLock(ledger, hypothesisIds[0]!);
    ledger.patch("hypothesis", hypothesisIds[1]!, { status: "proven", resolution_reason: "survived" });

    const verdict = terminationVerdict(ledger.projection, 2, DEFAULT_TERMINATION, DEFAULT_VERDICTS);
    expect(verdict.outcome).toBe("completed");
  });

  it("measures the floor against the same score the frontier is ranked by", () => {
    const { ledger, hypothesisIds } = newLedger();
    resolve(ledger, hypothesisIds[0]!);
    ...
    const blocked = terminationVerdict(ledger.projection, 1, DEFAULT_TERMINATION, DEFAULT_VERDICTS);
    expect(blocked.outcome).toBeNull();
    // Same frontier, a floor above every score: now nothing blocks.
    const cleared = terminationVerdict(ledger.projection, 1, { ...DEFAULT_TERMINATION, priority_floor: 99 }, DEFAULT_VERDICTS);
    expect(cleared.outcome).toBe("completed");
  });
});

describe("the budget checkpoint", () => {
  async function parkedHunt(overrides: { termination?: Partial<Termination> } = {}) {
    const { ledger, hypothesisIds } = newLedger(["a credential is used from new infrastructure"], {
      budgets: { max_iterations: 1, max_cost_usd: 10 },
      ...overrides,
    });
    const result = await controllerFor(ledger, [INVESTIGATE]).advanceIteration();
    return { ledger, hypothesisIds, result };
  }
  ...
  it("takes the outcome from the predicate, not from having run out of money", async () => {
    const { ledger, hypothesisIds } = newLedger(["a credential is used from new infrastructure"], {
      budgets: { max_iterations: 2, max_cost_usd: 10 },
    });
    await gapLock(ledger, hypothesisIds[0]!);
  ...
  it("still parks when the budget runs out with work left", async () => {
    const { ledger, result } = await parkedHunt();
    expect(result.hunt_status).toBe("parked");
    expect(ledger.projection.hunt.outcome).toBeNull();
  });
  ...
  it("un-parks on an extend that grants headroom", async () => {
    ...
    steer(ledger.path, "extend", "+3 iterations");
  ...
  it("clamps an extend to the hard ceiling and says that it clamped", async () => {
    const { ledger } = await parkedHunt({ termination: { hard_max_iterations: 3, hard_max_cost_usd: 12 } });
    steer(ledger.path, "extend", "+50 iterations and $500");

    await controllerFor(ledger, [INVESTIGATE]).advanceIteration();

    expect(ledger.projection.hunt.budgets).toEqual({ max_iterations: 3, max_cost_usd: 12 });
  ...
  it("stays parked when the clamp leaves no room to run", async () => {
    const { ledger } = await parkedHunt({ termination: { hard_max_iterations: 1, hard_max_cost_usd: 10 } });
    steer(ledger.path, "extend", "+5 iterations");
  ...
  it("keeps the hunt parked when the grant cannot be read", async () => {
    const { ledger } = await parkedHunt();
    steer(ledger.path, "extend", "give it a bit more room");
  ...
  it("ends budget_terminated when the operator accepts the stop", async () => {
    const { ledger, hypothesisIds } = await parkedHunt();
    steer(ledger.path, "conclude", "we are done spending on this");
...
describe("Finalize runs on every terminal path", () => {
  it.each([
    ["completed", async (ledger: Ledger, ids: string[]) => {
      resolve(ledger, ids[0]!);
      await controllerFor(ledger, [CONCLUDE]).advanceIteration();
    }],
    ["data_starved", async (ledger: Ledger, ids: string[]) => {
      await gapLock(ledger, ids[0]!);
      await controllerFor(ledger, [CONCLUDE]).advanceIteration();
    }],
    ["budget_terminated", async (ledger: Ledger) => {
      steer(ledger.path, "conclude", "accepted");
      await controllerFor(ledger, []).advanceIteration();
    }],
    ["aborted", async (ledger: Ledger) => {
      steer(ledger.path, "abort", "halted");
      await controllerFor(ledger, []).advanceIteration();
    }],
  ])("journals a report and writes one beside the ledger when a hunt ends %s", async (outcome, drive) => {
    const { ledger, hypothesisIds } = newLedger();
    await drive(ledger, hypothesisIds);

    expect(ledger.projection.hunt.outcome).toBe(outcome);

    const reports = finalized(ledger);
    expect(reports).toHaveLength(1);
    expect(reports[0]!.outcome).toBe(outcome);

    const path = reportPath(ledger.path);
    expect(existsSync(path)).toBe(true);
    expect(readFileSync(path, "utf8")).toMatch(new RegExp(`\\*\\*Outcome:\\*\\* ${outcome}`));
  });

  it("finalizes exactly once when a lower-precedence terminate follows", () => {
    const { ledger } = newLedger();
    const controller = controllerFor(ledger, []);
    controller.terminate("aborted");
    controller.terminate("completed");
  ...
  it("rebuilds the same report from the JSONL alone", async () => {
    const { ledger, hypothesisIds } = newLedger();
    await gapLock(ledger, hypothesisIds[0]!);
    await controllerFor(ledger, [CONCLUDE]).advanceIteration();

    // What `vigilhunt report` does: replay-derived, so it works on any ledger.
    const rebuilt = buildReport(Ledger.open(ledger.path).projection);
    expect(rebuilt).toEqual(finalized(ledger)[0]);
    expect(renderReport(rebuilt)).toBe(readFileSync(reportPath(ledger.path), "utf8"));
  });
  ...
  it("reports a parked hypothesis as backlog, not as a verdict", () => {
    const { ledger, hypothesisIds } = newLedger();
    resolve(ledger, hypothesisIds[0]!);
...
describe("termination is config", () => {
  it("ships the documented defaults and honours an override", () => {
    const shipped = loadConfig("vigil.config.yaml").termination;
    expect(shipped.priority_floor).toBe(DEFAULT_TERMINATION.priority_floor);
    expect(shipped.park_ttl_ms).toBe(DEFAULT_TERMINATION.park_ttl_ms);

    expect(parseConfig("rates: { input: 1, output: 1 }\ntermination: { priority_floor: 9 }\n").termination).toEqual({
      ...DEFAULT_TERMINATION,
      priority_floor: 9,
    });
  });
  ...
  it("refuses a ceiling under the budget it is meant to cap", () => {
    expect(() =>
      parseConfig("rates: { input: 1, output: 1 }\nbudgets: { max_iterations: 30 }\ntermination: { hard_max_iterations: 10 }\n"),
    ).toThrow(/below budgets.max_iterations/);
    expect(() => parseConfig("rates: { input: 1, output: 1 }\ntermination: { priority_floor: 0 }\n")).toThrow(
      /must be a positive number/,
    );
    expect(() => parseConfig("rates: { input: 1, output: 1 }\ntermination: { floor: 5 }\n")).toThrow(
      /unknown termination key/,
    );
  });
});
